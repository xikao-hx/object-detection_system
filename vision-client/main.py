...
class MainWindow(QMainWindow, Ui_MainWindow):
    ...
    def open_rtsp_input(self):
        # 实例化 rtsp_win.py 里的 Window：获取 rtsp 流路径
        # self.rtsp_window = Window() 
        # self.rtsp_window.show()

        # 手动直接指定
        self.yolo_predict.source = "rtsp://172.32.0.93:554/live/0"
        self.rtsp_window.rtspButton.clicked.connect(self.start_rtsp_inference)

    def start_rtsp_inference(self):
        # 获取输入框里的地址
        # url = self.rtsp_window.rtspEdit.text()
        # if url:
        #     self.yolo_predict.source = url

        self.rtsp_window.close()
        self.run_or_continue() # 触发推理
